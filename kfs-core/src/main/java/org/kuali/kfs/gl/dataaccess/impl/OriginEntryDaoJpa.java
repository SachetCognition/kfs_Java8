...
public class OriginEntryDaoJpa extends PlatformAwareDaoBaseOjb implements OriginEntryDao {
    ...
    /**
     * Constructs a OriginEntryDaoJpa instance
     */
    public OriginEntryDaoJpa() {
        super();
    }
    ...
    public KualiDecimal getGroupTotal(Integer groupId, boolean isCredit) {
        ...
        crit.addEqualTo(OriginEntryDaoJpa.ENTRY_GROUP_ID, groupId);
        if (isCredit) {
            crit.addEqualTo(OriginEntryDaoJpa.TRANSACTION_DEBIT_CREDIT_CODE, KFSConstants.GL_CREDIT_CODE);
        }
        else {
            crit.addNotEqualTo(OriginEntryDaoJpa.TRANSACTION_DEBIT_CREDIT_CODE, KFSConstants.GL_CREDIT_CODE);
        }
