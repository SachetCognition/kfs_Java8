...
public class SchedulerServiceImpl implements SchedulerService {
    ...
    protected void loadJob(JobDescriptor jobDescriptor) {
        ...
        if (SCHEDULED_GROUP.equals(jobDetail.getKey().getGroup())) {
            JobDetail unscheduledJob = JobBuilder.newJob(Job.class)
    ...
    protected void dropDependenciesNotScheduled() {
        try {
            List<String> scheduledGroupJobNames = new ArrayList<String>();
            for (JobKey jk : scheduler.getJobKeys(GroupMatcher.jobGroupEquals(SCHEDULED_GROUP))) {
                scheduledGroupJobNames.add(jk.getName());
            }
    ...
    public void addScheduled(JobDetail job) {
        try {
            JobDetail scheduledJob = JobBuilder.newJob(Job.class)
                    .withIdentity(job.getKey().getName(), SCHEDULED_GROUP)
                    .storeDurably(job.isDurable())
                    .requestRecovery(job.requestsRecovery())
                    .build();
    ...
    public List<String> getSchedulerGroups() {
        try {
            return new ArrayList<String>(scheduler.getJobGroupNames());
        }
        catch (SchedulerException ex) {
            throw new RuntimeException("Exception while obtaining job list", ex);
        }
    ...
    protected List<String> getJobNamesForScheduleJob() {
        ...
        try {
            for (JobKey jk : scheduler.getJobKeys(GroupMatcher.jobGroupEquals(SCHEDULED_GROUP))) {
                String scheduledJobName = jk.getName();
                if (scheduler.getTriggersOfJob(jk).size() == 0) {
